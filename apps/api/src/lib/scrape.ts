...
const generateM3U8 = (
  channelsMap: Map<string, ChannelSearchResult>,
  remote?: boolean
) => {
  ...
  for (const [category, channels] of categories.entries()) {
    for (const { name, infohash } of channels.sort((a, b) => {
      ...
      const streamUrl = !remote
        ? `http://192.168.1.254:6878/ace/getstream?id=${infohash}`
        : `https://stream.rhymecode.net/ace/getstream?id=${infohash}&p_token=xcdy2nk4.wxktwxqpkyfphk4itwcks4pekm`;
...
const extractLivetvsxLiveEvents = (page: string) => {
  ...
  const root = $(
    "body > table > tbody > tr > td:nth-child(2) > table > tbody > tr:nth-child(6) > td > table > tbody > tr > td:nth-child(2) > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td > table > tbody > tr > td > table > tbody > tr > td > table:nth-child(5) > tbody > tr > td:nth-child(2) > table:nth-child(2)"
  );
